...
void
raisepanic(struct tracest* st, char* message)
{
  cprintf("Traceback:\n");
  for (int i = 0; i < st->size; ++i)
  {
    cprintf("  File \"%s\", line %d, in %s\n", st->data[i].filename,
            st->data[i].line, st->data[i].signature);
  }
  cprintf("MESSAGE: %s\n", message);
}

struct tracest*
pushtracest(char* filename, const char* signature, uint line)
{
